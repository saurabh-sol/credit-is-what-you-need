...
import { costOfCredits, formatUsd, type TopUpConfig } from "@/lib/topup";
  ...
  return (
    <div className="card relative overflow-hidden p-7">
      ...
      <div className="mt-8 grid grid-cols-[1fr_auto_1fr] items-center gap-4 font-mono">
        <div>
          <p className="text-2xl font-semibold text-fog sm:text-3xl">{config ? formatUsd(costOfCredits(1_000, config)) : "$"}</p>
          <p className="mt-1 text-xs text-mist">{config ? "in USDG or ETH" : "USDG or ETH"}</p>
        </div>
        <ArrowRightIcon className="size-5 text-accent" />
        <div className="text-right">
          <p className="text-2xl font-semibold text-accent sm:text-3xl">{config ? "1,000" : "credits"}</p>
          <p className="mt-1 text-xs text-mist">{config ? "credits · $1 of AI usage" : "price set at launch"}</p>
        </div>
      </div>
